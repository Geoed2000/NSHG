            ...
            if (this.dataBuffer == null)
            {
                ...
                else
                {
                    ...
                    if (this.maxMessageSize > 0 && length > this.maxMessageSize)
                        throw new ProtocolViolationException("Message length " + length.ToString(CultureInfo.InvariantCulture) + " is larger than maximum message size " + this.maxMessageSize.ToString(CultureInfo.InvariantCulture));
